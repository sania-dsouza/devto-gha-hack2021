...
export class SetDayMealComponent extends BaseComponent {
  START() {
    return this.$send(YesNoOutput, { message: 'Welcome to NutriPlanner app! Do you want to save your cal intake today?' });
  }
  ...
  @Intents('NoIntent')
  dontSetCalIntake() {
    return this.$send({ message: 'Alright! You can invoke me again by saying "Alexa, open jovo test" ' });
  }
  ...
  finishDayMeal() {
    return this.$send({ message: 'I have saved your day meal for ' + moment().format("Do MMM YYYY") + '.' });
  }
